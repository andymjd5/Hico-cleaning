      ...
      <section className="flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {activeSignals.length > 0 && onCompleteAllSignals && (
              <button
                onClick={() => {
                  if (confirm("Voulez-vous marquer TOUTES les alertes en cours comme traitées/ramassées ?")) {
                    onCompleteAllSignals();
                  }
                }}
                className="px-2.5 py-1 rounded-xl text-xs font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 hover:bg-emerald-500/30 transition-all cursor-pointer flex items-center gap-1.5 shadow-sm active:scale-95"
                title="Valider le ramassage de toutes les alertes enregistrées"
              >
                <CheckCircle2 size={13} />
                <span>Tout marquer comme traité</span>
              </button>
            )}
          </div>
          <button 
            onClick={() => onNavigate('dechets_map')}
            className="text-xs font-bold text-primary hover:text-on-background flex items-center gap-1 cursor-pointer transition-colors self-end sm:self-auto"
          >
            Carte interactive
            <ArrowRight size={14} />
          </button>
            ...
            {activeSignals.map((sig) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Éboueur Assignment controls & Resolution buttons */}
                  <div className="mt-1 pt-3 border-t border-outline-variant/40 flex flex-col gap-2">
                    {sig.status === 'assigned' ? (
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
                        <div className="flex items-center gap-2 text-xs font-bold text-amber-400 bg-amber-950/40 p-2 rounded-xl border border-amber-500/20 flex-1 min-w-0">
                          <Truck size={16} className="shrink-0 animate-pulse text-amber-400" />
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {onCompleteSignal && (
                            <button
                              onClick={() => {
                                if (confirm("Confirmer le ramassage et marquer cette alerte comme TRAITÉE / COMPLÉTÉE ?")) {
                                  onCompleteSignal(sig.id);
                                }
                              }}
                              className="px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-extrabold flex items-center gap-1.5 shadow-md active:scale-95 transition-all cursor-pointer"
                              title="Marquer cette poubelle comme ramassée"
                            >
                            ...
                            <button
                              onClick={() => {
                                if (confirm("Voulez-vous supprimer cette alerte ?")) {
                                  onCancelSignal(sig.id);
                                }
                              }}
                              className="p-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-semibold flex items-center justify-center cursor-pointer transition-colors"
                              title="Annuler/Supprimer l'alerte"
                            >
                              <Trash2 size={14} />
                            </button>
                          )}
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5 justify-end">
                          {onCompleteSignal && (
                            <button
                              onClick={() => {
                                if (confirm("Marquer directement cette alerte comme TRAITÉE / RAMASSÉE ?")) {
                                  onCompleteSignal(sig.id);
                                }
                              }}
                              className="px-2.5 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-400 text-xs font-bold flex items-center gap-1 cursor-pointer transition-colors shrink-0"
                              title="Marquer directement comme ramassée"
                            >
                              <CheckCircle2 size={14} />
                          ...
                          {onCancelSignal && (
                            <button
                              ...
                              className="p-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-semibold flex items-center justify-center cursor-pointer transition-colors shrink-0"
                              title="Annuler/Supprimer l'alerte"
                            >
                              <Trash2 size={14} />
                            </button>
